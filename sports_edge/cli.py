...
from .goal_audit import DURABLE_DAILY_PROOF_PATH, LIVE_SOURCE_PROOF_PATH, POSTGRES_PROOF_PATH, PRODUCTION_CRON_PROOF_PATH, build_goal_audit
...
from .proof_capture import load_json_file, write_durable_daily_proof, write_live_source_proof, write_production_cron_proof
...
def run_durable_daily_proof(evidence_in: str, proof_out: str, dry_run: bool) -> int:
    evidence = load_json_file(evidence_in)
    payload = write_durable_daily_proof(evidence, proof_out=proof_out, dry_run=dry_run)
    print(json.dumps(payload, indent=2, sort_keys=True))
    return 0 if payload.get("ok") else 1
...
def main() -> int:
    ...
    daily_proof_parser = subparsers.add_parser(
        "durable-daily-proof",
        help="Build sanitized durable daily write proof from approved evidence JSON without writing state",
    )
    daily_proof_parser.add_argument("--evidence-in", required=True, help="Path to operator-approved sanitized durable daily evidence JSON")
    daily_proof_parser.add_argument(
        "--proof-out",
        default=DURABLE_DAILY_PROOF_PATH,
        help=f"Path for sanitized proof output, default {DURABLE_DAILY_PROOF_PATH}",
    )
    daily_proof_parser.add_argument("--dry-run", action="store_true", help="Validate and print proof without writing it")
    ...
    if args.command == "durable-daily-proof":
        return run_durable_daily_proof(args.evidence_in, args.proof_out, args.dry_run)
